...
Section Permissions.
  ...
  Record perm : Type :=
    {
      ...
      (** Invariant on configs *)
      spred : config -> Prop;
      spred_respects1 : forall x y, guar x y -> spred x -> spred y;
      spred_respects2 : forall x y, rely x y -> spred x -> spred y;
    }.
  ...
  (* Proof. *)
  (*   repeat intro. subst. apply H. 2: auto. *)
  (* Qed. *)
  ...
  Qed.
  Ltac respects := try solve [eapply pre_respects; eauto];
                   try solve [eapply spred_respects1; eauto];
                   try solve [eapply spred_respects2; eauto].

  (*
  Definition join_guar p q := clos_trans _ (fun x y => (guar p x y) \/ (guar q x y)).
  ...
  Next Obligation.
    ...
    destruct H
    destruct H0 as [? | [? | [z [? ?]]]].
    - destruct H. left. eapply spred_respects1; eauto.
      right; right. exists x. split; auto. constructor 1. auto.
    - destruct H. 2: { right. left. eapply spred_respects1; eauto. }
      right; right. exists x. split; auto. constructor 1. auto.
  ...
  Proof.
    ...
    - destruct H as [? | [? | [? ?]]]; auto.
      + induction H0; auto.
        2: { transitivity y; auto. apply IHclos_trans2.
             fold (join_guar p q) in H0_. apply join_perm_obligation_4 in H0_; auto.

      + destruct H0, H. auto. auto.
        (* * apply guar_inc0; auto. *)
      + transitivity y; auto. apply IHclos_trans2.
        fold (join_guar p q) in H0_. apply join_perm_obligation_4 in H0_; auto.
    - destruct H; auto.
  Qed.
  ...
  Next Obligation.
    induction H; auto.
    destruct H0 as [? | [? | [z [? ?]]]].
    - destruct H; try solve [left; eapply pre_respects; eauto].
      right; right. exists x. split; auto. constructor 1. auto.
    - destruct H; try solve [right; left; eapply pre_respects; eauto].
      right; right. exists x. split; auto. constructor 1. auto.
    - destruct H; right; right; exists z; split; auto;
        econstructor 2; eauto; constructor 1; auto.
  Qed.
  Next Obligation.
    split; eapply spred_respects1; eauto.
  Qed.
...
*)
  ...
  Lemma separate_antimonotone : forall p q r,
      p ⊥ q -> r <= q -> p ⊥ r.
  Proof.
    intros. constructor.
  ...
  Next Obligation.
    ...
    - repeat intro. econstructor 3; eauto.

      (* destruct H, H0; subst; auto. *)
      (* right. destruct H, H0; auto. split; auto. *)
      (* econstructor 2; eauto. *)
  Qed.
  Next Obligation.
  ...
  Next Obligation.
    ...
    - destruct H; auto.
      + destruct H; split; auto.
        eapply spred_respects1; eauto.
      + destruct H; split; auto.
        eapply spred_respects1; eauto.
    - split; auto.
    (* - destruct H2. *)
    (*   + split. eapply spred_respects1; eauto. *)
    (*     apply H in H2; auto. eapply spred_respects2; eauto. *)
    (*   + split. *)
    (*     apply H in H2; auto. eapply spred_respects2; eauto. *)
    (*     eapply spred_respects1; eauto. *)
    - apply IHclos_refl_trans2; auto; apply IHclos_refl_trans1; auto.
  Qed.
  Next Obligation.
      split; eapply spred_respects2; eauto.
  Qed.

  (*   induction H; auto. *)
  (*   2: { apply IHc2; auto. apply IHc1; auto. apply IHc1; auto. } *)
  (*   split; auto. *)
  (*   - destruct H. *)
  (*     + eapply spred_respects1; eauto. *)
  (*     + eapply spred_respects1; eauto. apply H2 in H; auto. eapply spred_respects2; eauto. *)
  (*   - destruct H. *)
  (*     + apply H2 in H; auto. eapply spred_respects2; eauto. *)
  (*     + eapply spred_respects1; eauto. *)
  (* Qed. *)
  (* Next Obligation. *)
  (*   split; [| split]; auto; eapply spred_respects2; eauto. *)
  (* Qed. *)
  ...
  Proof.
    ...
    - induction H2.
      + destruct H2.
        ...
        split. apply H; auto. apply H1. apply H0. auto.

        constructor 1. right. destruct H2.
        split. apply H0; auto. apply H1. apply H. auto.
      + constructor 2; auto.
      + econstructor 3; eauto. apply IHclos_refl_trans2.
        eapply sep_conj_perm_obligation_4; eauto.


      (* destruct H2; subst; auto. right. *)
      (* destruct H2 as (? & ?). destruct H1. (* apply H2 in H1. apply H3 in H5.  *)(* clear H2 H3. *) *)
      (* split. intros. apply H. auto. *)
      (* split. intros. apply H0. auto. *)
      (* (* clear H1 H2 H3 H5. *) *)
      (* revert H1 H2 H3 H5. *)
      (* induction H4; intros; auto. *)
      (* + destruct H1. *)
      (*   * constructor 1. left. apply H; auto. *)
      (*   * constructor 1. right. apply H0; auto. *)
      (* + admit. *)
        (* econstructor 2. *)
        (* * apply IHclos_trans1; auto. intros _. *)
        (* * apply IHclos_trans2; auto. *)
      (* 2: { econstructor 2; eauto. eapply IHclos_trans1. *)
      (*      eapply sep_conj_perm_obligation_4; eauto. } *)
      (* constructor 1. destruct H2. *)
      (* + left. apply H; auto. apply H1. *)
      (* + right. apply H0; auto. apply H1. *)
    - destruct H1 as (? & ? & ?); split; [| split]; eauto.
      eapply separate_antimonotone; eauto. symmetry. symmetry in H3.
      eapply separate_antimonotone; eauto.
    - destruct H1. split. apply H. auto. apply H0. auto.
      ...
      (*   eapply separate_antimonotone in H3; eauto. *)
      (*   symmetry in H3. *)
      (*   eapply separate_antimonotone in H3; eauto. *)
      (*   2: { intros ? []. split; auto. apply H0. auto. } *)
      (*   symmetry in H3. *)
  Qed.
